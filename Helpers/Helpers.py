...
def getEndPoint(angle: int, length: int, pos: Tuple[int, int]) -> Tuple[float, float]:
...
def adjustIntersect(intersectPoint: Tuple[int, int], camera: Type[Camera]) -> Tuple[float, float]:
...
def getCircumference(diameter: int) -> float:
